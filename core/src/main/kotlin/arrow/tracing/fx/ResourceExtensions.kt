...
/**
 * Creates a single threaded [CoroutineContext] as a [Resource]. Upon release an orderly shutdown of
 * the [ExecutorService] takes place in which previously submitted tasks are executed, but no new
...
public suspend fun ResourceScope.executor(
  ...
  install({ create() }) { s: ExecutorService, _: ExitCase ->
      s.shutdown()
...
public fun executor(
  ...
  closingDispatcher: CoroutineDispatcher = Dispatchers.IO,
  create: suspend () -> ExecutorService,
): Resource<ExecutorCoroutineDispatcher> = resource { executor(timeout, closingDispatcher, create) }
...
public suspend fun <A : Closeable> ResourceScope.closeable(
  ...
  install({ closeable() }) { s: A, _: ExitCase -> withContext(closingDispatcher) { s.close() } }

public fun <A : Closeable> closeable(
  closingDispatcher: CoroutineDispatcher = Dispatchers.IO,
  closeable: suspend () -> A,
): Resource<A> = resource { closeable(closingDispatcher, closeable) }
...
public suspend fun <A : AutoCloseable> ResourceScope.autoCloseable(
  ...
  install({ autoCloseable() }) { s: A, _: ExitCase -> withContext(closingDispatcher) { s.close() } }

public fun <A : AutoCloseable> autoCloseable(
  closingDispatcher: CoroutineDispatcher = Dispatchers.IO,
  autoCloseable: suspend () -> A,
): Resource<A> = resource { autoCloseable(closingDispatcher, autoCloseable) }
...
public fun singleThreadContext(name: String): Resource<ExecutorCoroutineDispatcher> = resource {
...
@OptIn(DelicateCoroutinesApi::class)
@ResourceDSL
public suspend fun ResourceScope.fixedThreadPoolContext(
  nThreads: Int,
  name: String
): ExecutorCoroutineDispatcher = closeable { newFixedThreadPoolContext(nThreads, name) }

public fun fixedThreadPoolContext(
  nThreads: Int,
  name: String
): Resource<ExecutorCoroutineDispatcher> = resource { fixedThreadPoolContext(nThreads, name) }
