...
import arrow.tracing.model.toTraceValue
  ...
  object : OpenCensusSpan {
    ...
    override suspend fun put(fields: List<Pair<String, TraceValue>>): Unit =
      fields.forEach { (key, traceValue) ->
        when (traceValue) {
          is BooleanValue -> span.putAttribute(key, AttributeValue.booleanAttributeValue(traceValue.value))
          is CharValue -> span.putAttribute(key, AttributeValue.stringAttributeValue("${traceValue.value}"))
          is DoubleValue -> span.putAttribute(key, AttributeValue.doubleAttributeValue(traceValue.value))
          is FloatValue -> span.putAttribute(key, AttributeValue.doubleAttributeValue(traceValue.value.toDouble()))
          is IntValue -> span.putAttribute(key, AttributeValue.longAttributeValue(traceValue.value.toLong()))
          is LongValue -> span.putAttribute(key, AttributeValue.longAttributeValue(traceValue.value))
          is ShortValue -> span.putAttribute(key, AttributeValue.longAttributeValue(traceValue.value.toLong()))
          is StringValue -> span.putAttribute(key, AttributeValue.stringAttributeValue(traceValue.value))
          else -> Unit
        }
      }
    ...
    override fun traceId(): String =
      span.context.traceId.toLowerBase16()

    override suspend fun spanId(): String =
      span.context.spanId.toLowerBase16()
...
/**
 * @throws SpanContextParseException or NoSuchElementException, NullPointerException when headers are incomplete or invalid
 */
...
private val contextSetter: TextFormat.Setter<MutableMap<String, String>>
  get() = object : TextFormat.Setter<MutableMap<String, String>>() {
...
private val contextGetter: Getter<Kernel>
  get() = object : Getter<Kernel>() {
    override fun get(carrier: Kernel, key: String): String? =
      carrier.headers[key]
  }
