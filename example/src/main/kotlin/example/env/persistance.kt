...
import example.repo.UserId
import example.sqldelight.SqlDelight
import java.time.OffsetDateTime
import javax.sql.DataSource

public suspend fun ResourceScope.hikari(env: Env.DataSource): HikariDataSource = autoCloseable {
  HikariDataSource(
    HikariConfig().apply {
      jdbcUrl = env.url
...
public suspend fun ResourceScope.sqlDelight(dataSource: DataSource): SqlDelight {
  val driver = closeable { dataSource.asJdbcDriver() }
  SqlDelight.Schema.create(driver)
  return SqlDelight(
    driver,
    // Articles.Adapter(articleIdAdapter, userIdAdapter, offsetDateTimeAdapter,
